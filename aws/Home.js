...
function Home() {
        ...
        try {
            ...
            for (const segment of audioSegments) {
                ...
                if (segment.type === 'uploaded') {
                    blob = dataURLtoBlob(segment.url); // Convert uploaded audio URL to Blob
                } else if (segment.type === 'recorded' && segment.blob) {
                    blob = segment.blob; // Directly use recorded blob
                }
    
                if (blob) {
                    formData.append('files', blob, segment.filename); // Append file to form data
                }
            }
            ...
            if (!response.ok) {
                ...
                throw new Error(`Failed to compile audio. Backend error: ${errorText}`);
            }
    
            console.log('Response received:', response);
            const result = await response.json(); // Assuming the backend returns JSON
            setCompilationDone(true);
            setCompiledAudioUrl(result.compiledFile); // Assuming 'compiledFile' is the key in response
            console.log('Audio compiled successfully:', result);
    
        } catch (err) {
            console.error('Error compiling audio:', err);
            setError(`Failed to compile audio: ${err.message}`);
        }
    };
    ...
    const uploadModel = async () => {
        try {
            console.log('Uploading model for training...');
    ...
    return (
        <div className="App">
            ...
            <div className="App-content">
                        ...
                        {audioSegments.map(segment => (
                            <div key={segment.id} className="audio-segment">
                                ...
                                <input
                                    ...
                                    className="tag-input"
                                />
                                <button onClick={() => discardAudio(segment.id)} className="discard-btn">
                                    Discard
                                </button>
                            </div>
                        ))}
                ...
                <div className="App-buttons">
                    <button onClick={compileAudio}>Compile Audio</button>
                    {compilationDone && (
                        <button onClick={uploadModel}>Upload Model</button>
                    )}
                </div>
    
                {compilationDone && compiledAudioUrl && (
                    <div className="compiled-audio">
                        <h3>Compiled Audio</h3>
                        <audio controls>
                            <source src={compiledAudioUrl} />
                        </audio>
                    </div>
                )}
            </div>
